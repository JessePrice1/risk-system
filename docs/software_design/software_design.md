...
### Purpose
Stores all project documentation including model documentation, software design, test plans, and test results.

### Assumptions
- Documentation is manually maintained and updated
- Used only for reporting, not computation
...
### Purpose
Entry point of the system that orchestrates data loading, model execution, risk computation, and result generation.
...
- Output: full risk analysis results (VaR, ES, backtesting outputs)
...
### Interface
- Input: manual configuration (Python variables)
...
### Purpose
Computes asset and option pricing using financial models (e.g., Black-Scholes, return calculations).
...
- Output: computed prices and returns
...
### Purpose
Implements risk metrics including VaR and Expected Shortfall using historical, parametric, and Monte Carlo methods.
...
### Interface
- Input: generic numerical arrays / DataFrames
- Output: computed statistical outputs (mean, variance, covariance, plots)
...
## **Covariance Module (`covariance.py`)**

### Purpose
Covariance captures portfolio risk from joint movements. It is important for calculating Parametric VaR.

### Assumptions
- Linear dependence between asset returns
- Historical relationships are stable over time
- Market conditions remain statistically consistent over estimation window
...
- Output: `cov_matrix` (N × N)
  - Pairwise covariance between assets

### Limitations
- Sensitive to outliers and extreme values
- Assumes stationarity of return relationships
